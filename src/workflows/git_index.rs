...
fn run_rebuild_git(
    ...
    let total_commits = git::estimate_commit_count(&request.repo_path, git_config, None)?;
    let pb1 = Progress::new(total_commits as u64, "Walking commits", request.verbose);
    let t1 = std::time::Instant::now();
    let docs = git::index_git_history(&request.repo_path, git_config, None, true, request.verbose, Some(&pb1))?;
    pb1.finish();
    ...
    let head_commit = git::resolve_head_commit(&request.repo_path, &git_config.branch)?;
...
fn run_incremental_git(
    ...
    let total_new = git::estimate_commit_count(&request.repo_path, git_config, last_commit.as_deref())?;
